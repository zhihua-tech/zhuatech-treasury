...
class TreasuryApiIntegrationTests {
    ...
    @Test void adminCanAssessLiquidityCoverage() throws Exception {
        mvc.perform(post("/api/admin/liquidity-coverage").with(httpBasic("admin", "admin123"))
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"availableCash\":5000000,\"committedFacilities\":2000000,\"paymentsDueSevenDays\":8000000,\"receivablesSevenDays\":1000000,\"restrictedCash\":1000000,\"minimumBuffer\":1000000}"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data.surplus").value(-2000000.0))
            .andExpect(jsonPath("$.data.coverageRate").value(77.8))
            .andExpect(jsonPath("$.data.status").value("GAP"));
    }
}
